...
class PDE_Solver():
    ...
    def solver_stokes(self,eta_wedge,u_n,collect_bc,I_Field):
        ...
        for cell in cells(self.mesh):
            ...
            cell_dofs = self.dm_W_u_p.cell_dofs(cell.index())
            if subdomain_index != 18:
                u_new.vector()[cell_dofs[:]] = u_n.vector()[cell_dofs[:]]
        
        # update inflow/outflow bc
        self.update_inflow_boundary(u_new)
        return u_new
    ...
    def solver_adv_diff(self,u_n,T_n,J,D_Field):
        ...
        T_base_exp = Expression("T_base", T_base=self.param.Tb, degree=self.param.T_CG_order)
        ...
        F_temp += (1/self.param.dt) * Constant(self.param.rho_slab)*Constant(self.param.cp)*(inner(self.v_T,self.phi_T) - inner(self.v_T, T_n))*dx
        
        # F_temp = Constant(self.param.rho_slab)*Constant(self.param.cp)*(inner(self.v_T,self.phi_T) - inner(self.v_T, T_n))*dx
        # F_temp += self.param.dt*inner(grad(self.v_T), Constant(self.param.k_slab)*grad(self.phi_T))*self.dx(17)
        # F_temp += self.param.dt*inner(grad(self.v_T), Constant(self.param.k_mantle)*grad(self.phi_T))*self.dx(18)
        # F_temp += self.param.dt*inner(grad(self.v_T), Constant(self.param.k_crust)*grad(self.phi_T))*self.dx(19)
        # F_temp += self.param.dt*Constant(self.param.rho_slab)*Constant(self.param.cp)*self.v_T*inner(u_n, grad(self.phi_T))*self.dx(17)
        # F_temp += self.param.dt*Constant(self.param.rho_mantle)*Constant(self.param.cp)*self.v_T*inner(u_n, grad(self.phi_T))*self.dx(18)
    ...
    def run_solver(self):
        ...
        self.P_T = FiniteElement("CG", self.mesh.ufl_cell(), self.param.T_CG_order)
        self.W_T = FunctionSpace(self.mesh, self.P_T)
        self.v_T = TestFunction(self.W_T)
        self.phi_T = TrialFunction(self.W_T)

        # create ic for T

        print('self.param.z_bc',self.param.z_bc)

        class T_IC(UserExpression):
            def __init__(self, param, z_top, **kwargs):
                super(T_IC, self).__init__(**kwargs)
                self.param = param
                self.z_top = z_top
            def eval(self, values, x):
                if self.param.z_bc is not None:
        ...
        tic = T_IC(self.param,self.z_top)
        T_old = Function(self.W_T)
        # Tinit = Expression("Ts + (Tb - Ts)*abs(x[1]-4.65)/abs(40.0)", Ts=self.param.Ts, Tb=self.param.Tb, degree = self.param.T_CG_order)
        Tinit = Expression("T", T=tic, degree = self.param.T_CG_order)
        T_old.interpolate(Tinit)

        t_IC_file_pvd = File(os.path.join(self.output_dir,"temperature_IC.pvd"))
        t_IC_file_pvd << T_old
        ...
        # T_old = Function(self.W_T)
        # T_old.assign(T_0)
        # T_old.vector()[:] = T_0.vector()[:]

        #isoviscous solve
        _,eta_wedge = self.isoviscous()
        u_0 = self.solver_stokes(eta_wedge,u_n,collect_bc,I_Field)
        T_pm1, res_T = self.solver_adv_diff(u_0,T_old,J_idx,D_Field)

        # T_pm1 = Function(self.W_T)
        # T_pm1.vector()[:] = T_p0.vector()[:]
        ...
        for ts in range((self.param.n_ts)):
            ...
            if self.param.viscosity_type == 'isoviscous':
                ...
                T_p, res_T = self.solver_adv_diff(u_n,T_old,J_idx,D_Field)
            else:
                i = 0
                while i<self.param.n_picard_it:
                    ...
                    if self.param.viscosity_type == 'diffcreep':
                        eta_wedge = self.diffcreep(T_pm1)
                    elif self.param.viscosity_type == 'disccreep':
                        eta_wedge = self.disccreep(u_n,T_pm1)
                    elif self.param.viscosity_type == 'mixed':
                        eta_wedge = self.mixed(u_n,T_pm1)
                    else:
                    ...
                    diff_i = (T_p.vector() - T_pm1.vector()).norm('l2')
                    print('diff between nonlinear iterations:', diff_i)
                    if diff_i < self.param.diff_tol:
                        print('diff is below tolerance, moving to next timestep')
                        break
                    T_pm1.vector()[:] = T_p.vector()[:]
                    i += 1
        ...
        self.write(np.array(T_old.vector()),os.path.join(self.output_dir, "temperature.pkl"))
